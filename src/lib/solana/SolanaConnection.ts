...
export class SolanaConnection extends Connection {
  constructor(
    ...
    debug(
      "NEXT_PUBLIC_SOLANA_RPC_MONIKER:",
      process?.env?.NEXT_PUBLIC_SOLANA_RPC_MONIKER as Cluster,
    );
    debug(
    ...
    // auto select a solana cluster to select to, while using "devnet" as the fallback
    if (!endpoint) {
      endpoint =
        // process?.env?.NEXT_PUBLIC_SOLANA_RPC_URL ||
        clusterApiUrl(
